...
function renderProductPage(slug){
  const p=GOLD_SEO_PRODUCTS[slug]; 
  if(!p)return null;
  const title=`${p.name} Ne Kadar? Güncel Alış Satış Fiyatı | Bugün Altın`; 

  const desc=`${p.name} ne kadar? Güncel ${p.short} alış ve satış fiyatını, şehir bazlı altın fiyatlarını ve hızlı altın hesaplama aracını Bugün Altın'da takip edin.`;

  const canonical=`${BASE}/${slug}`;

  const faqs=[
    [`${p.name} ne kadar?`,`${p.name} güncel alış ve satış fiyatı canlı fiyat bölümünde gösterilir.`],
...
function renderCityProductPage(citySlug,productSlug){
  ...
  const desc=`${cityName} ${p.name} ne kadar? Güncel ${p.short} alış ve satış fiyatlarını, şehir bazlı altın fiyatlarını ve hızlı altın hesaplama aracını Bugün Altın'da takip edin.`;

  const canonical=`${BASE}/${citySlug}-${productSlug}`;

  const faqs=[
    [
      `${cityName} ${p.name} ne kadar?`,
  ...
  return renderTemplate({
    ...
    seoContent:`
      <section class="seo-content">
        <h1>${cityName} ${p.name} Ne Kadar?</h1>

        <p>
          ${cityName} ${p.name} ne kadar sorusunun güncel yanıtını
